...
# No entering and leaving messages
...
# optional sanitizers: run make SANI=no to deactivate them
ifneq ($(strip $(SANI)), no)
  CFLAGS += -fsanitize=address -fsanitize=undefined -fsanitize=integer-divide-by-zero -fsanitize=unreachable -fsanitize=signed-integer-overflow #-fsanitize=thread
endif
...
# this checks the program for leaks using valgrind
leaks:
	valgrind --leak-check=yes ./$(TARGET_EXECS)

# this checks the program mainly for pointer/alocation issues
cpp:
	@cppcheck --enable=all --language=c --std=c99 -v (TARGET_EXECS)

# this starts up most of the gdb automatically
gdb:
	$(MAKE)
	gdb ./$(TARGET_EXECS) < gdb.txt
